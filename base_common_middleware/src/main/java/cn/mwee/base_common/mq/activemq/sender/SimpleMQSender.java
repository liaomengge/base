...
import cn.mwee.base_common.mq.activemq.processor.TraceMessagePostProcessor;
...
import static cn.mwee.base_common.mq.consts.MQConst.ActiveMQ.BACKUP_QUEUE_SUFFIX;
...
public class SimpleMQSender extends BaseMQSender {
    ...
    public SimpleMQSender(PooledConnectionFactory connectionFactory, ActiveMQMonitor activeMQMonitor) {
    ...
    public void convertAndSend(int queueHash, Object message, MessagePostProcessor messagePostProcessor) {
        ...
        activeMQMonitor.monitorCount(MetricsConst.ENQUEUE_COUNT + "." + queueName);
        if (autoBackup) {
            jmsTemplate.convertAndSend(queueName + BACKUP_QUEUE_SUFFIX, message, messagePostProcessor);
        }
    }
