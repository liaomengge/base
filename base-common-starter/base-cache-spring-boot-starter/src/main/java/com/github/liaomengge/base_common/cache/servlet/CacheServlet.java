package com.github.liaomengge.base_common.cache.servlet;

import com.github.liaomengge.base_common.cache.CachePoolHelper;
import com.github.liaomengge.base_common.cache.servlet.enums.EvictTypeEnum;
...
public class CacheServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (EvictTypeEnum.LEVEL1.name().equalsIgnoreCase(type)) {
        ...
        } else if (EvictTypeEnum.LEVEL2.name().equalsIgnoreCase(type)) {
            cachePoolHelper.evictLevel2(key);
            LyWebUtil.renderJson(resp, respBody.setMsg("[evict]二级缓存成功"));
        } else if (EvictTypeEnum.ALL.name().equalsIgnoreCase(type)) {
            String region = MapUtils.getString(paramMap, "region");
            if (StringUtils.isNotBlank(region)) {
