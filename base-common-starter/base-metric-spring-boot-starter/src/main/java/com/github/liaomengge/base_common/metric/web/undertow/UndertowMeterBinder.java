...
public class UndertowMeterBinder implements MetricsCollector, ApplicationListener<ApplicationStartedEvent> {
    ...
    private void registerGlobalRequestMetrics(MeterRegistry registry, String servletName,
        ...
        bindTimer(registry, servletName, MetricsConst.UNDERTOW_PREFIX + "requests", metricsHandler,
                val -> val.getMetrics().getTotalRequests(), val2 -> val2.getMetrics().getMinRequestTime());
        bindTimeGauge(registry, servletName, MetricsConst.UNDERTOW_PREFIX + "request.time.max", metricsHandler,
                val -> val.getMetrics().getMaxRequestTime());
        bindTimeGauge(registry, servletName, MetricsConst.UNDERTOW_PREFIX + "request.time.min", metricsHandler,
                val -> val.getMetrics().getMinRequestTime());
        bindCounter(registry, servletName, MetricsConst.UNDERTOW_PREFIX + "request.errors", metricsHandler,
            ...
            try {
                ...
                if (CollectionUtils.isNotEmpty(objectNames)) {
                    objectNames.stream().sorted(Comparator.reverseOrder()).findFirst().ifPresent(val -> {
                        bindGauge(registry, MetricsConst.UNDERTOW_PREFIX + "io.thread.count", BaseUnits.THREADS,
                                mBeanServer, toDoubleFunction(mBeanServer, val, "IoThreadCount"));
                        bindGauge(registry, MetricsConst.UNDERTOW_PREFIX + "worker.queue.size", BaseUnits.THREADS,
                                mBeanServer, toDoubleFunction(mBeanServer, val, "WorkerQueueSize"));
                        bindGauge(registry, MetricsConst.UNDERTOW_PREFIX + "core.worker.pool.size", BaseUnits.THREADS,
                                mBeanServer, toDoubleFunction(mBeanServer, val, "CoreWorkerPoolSize"));
                        bindGauge(registry, MetricsConst.UNDERTOW_PREFIX + "max.worker.pool.size", BaseUnits.THREADS,
                                mBeanServer, toDoubleFunction(mBeanServer, val, "MaxWorkerPoolSize"));
                        if (Version.VERSION.compareTo("3.5.0.Final") >= 0) {
                            bindGauge(registry, MetricsConst.UNDERTOW_PREFIX + "busy.worker.thread.count",
                                    BaseUnits.THREADS,
