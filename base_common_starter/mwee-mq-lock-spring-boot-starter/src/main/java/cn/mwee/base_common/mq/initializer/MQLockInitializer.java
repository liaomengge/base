package cn.mwee.base_common.mq.initializer;

import cn.mwee.base_common.helper.mail.MailHelper;
import cn.mwee.base_common.helper.zk.AbstractLock;
import cn.mwee.base_common.mq.MQLockProperties;
import cn.mwee.base_common.mq.MQLockProperties.PrototypeProperties;
import cn.mwee.base_common.mq.rabbitmq.receiver.BaseMQReceiver;
...
public class MQLockInitializer extends AbstractLock implements EnvironmentAware, ApplicationContextAware,
    ...
    private Map<String, cn.mwee.base_common.mq.activemq.receiver.BaseMQReceiver> baseActiveMQReceiverMap;
        ...
        try {
            ...
            if (StringUtils.equalsIgnoreCase(RABBITMQ, mqType)) {
                ...
                baseActiveMQReceiverMap =
                        applicationContext.getBeansOfType(cn.mwee.base_common.mq.activemq.receiver.BaseMQReceiver.class);
                Optional.ofNullable(baseActiveMQReceiverMap).ifPresent(val -> val.forEach((key, value) -> value.stop()));
                Optional.ofNullable(beanNames).ifPresent(val -> val.stream().forEach(val2 -> {
                    cn.mwee.base_common.mq.activemq.receiver.BaseMQReceiver baseMQReceiver =
