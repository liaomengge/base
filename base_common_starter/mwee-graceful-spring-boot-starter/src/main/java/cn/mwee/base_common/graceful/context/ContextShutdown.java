package cn.mwee.base_common.graceful.context;

import cn.mwee.base_common.graceful.GracefulProperties;
...
import static cn.mwee.base_common.graceful.consts.GracefulConst.CHECK_INTERVAL;
...
public class ContextShutdown implements ApplicationListener<ContextClosedEvent>, ApplicationContextAware {
    ...
    private void shutdown(String threadName, ThreadPoolTaskExecutor taskExecutor) {
        ...
        try {
            for (int remaining = this.gracefulProperties.getTimeout(); remaining > 0; remaining -= CHECK_INTERVAL) {
                try {
                    if (taskExecutor.getThreadPoolExecutor().awaitTermination(Math.min(remaining, CHECK_INTERVAL),
                            TimeUnit.SECONDS)) {
                        continue;
                    }
    ...
    private void shutdown(String threadName, ThreadPoolExecutor executor) {
        ...
        try {
            for (int remaining = this.gracefulProperties.getTimeout(); remaining > 0; remaining -= CHECK_INTERVAL) {
                try {
                    if (executor.awaitTermination(Math.min(remaining, CHECK_INTERVAL),
                            TimeUnit.SECONDS)) {
                        continue;
                    }
                } catch (InterruptedException e) {
